...
function Modal({ open, onClose, title, children }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={onClose}>
      ...
      <div
        className="relative bg-[#0f0f18] border border-white/10 rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto p-6"
  ...
  return (
    <Modal open={open} onClose={onClose} title="Add Note">
      <textarea
        ...
        rows={4}
        className="w-full bg-dark-200/50 border border-white/10 rounded-xl px-4 py-3 text-white placeholder-gray-500 focus:outline-none focus:border-[#13b973]/50 resize-none"
        placeholder="Enter note..."
      />
      <div className="flex justify-end gap-3 mt-4">
        <button onClick={onClose} className="px-4 py-2 text-gray-400 hover:text-white transition-colors">Cancel</button>
        <button onClick={handleSave} className="px-5 py-2 bg-[#13b973] hover:bg-[#13b973]/80 text-white rounded-xl font-medium transition-colors">Save Note</button>
      </div>
    </Modal>
  )
}
...
function ClientCard({ client, onUpdate, onDelete }) {
  ...
  const countdownColor = days === null ? 'text-gray-500' : days < 0 ? 'text-red-400' : days <= 3 ? 'text-yellow-400' : 'text-green-400'
  ...
  return (
    <>
      <div className="bg-dark-100/50 border border-white/5 rounded-xl p-5 hover:border-white/10 transition-all group">
        <div className="flex items-start justify-between mb-3">
          <div className="flex items-center gap-2.5">
            <button onClick={cycleStatus} title={`Status: ${STATUS_COLORS[client.status].label} (click to cycle)`}>
              <span className={`inline-block w-3 h-3 rounded-full ${STATUS_COLORS[client.status].dot} ring-2 ring-white/10 cursor-pointer hover:ring-white/30 transition-all`} />
            </button>
            <h3 className="text-white font-semibold text-lg">{client.clientName}</h3>
          </div>
          <button onClick={() => onDelete(client.id)} className="text-gray-600 hover:text-red-400 transition-colors opacity-0 group-hover:opacity-100 text-lg" title="Delete">&times;</button>
        </div>

        <p className="text-gray-400 text-sm mb-1">Project: <span className="text-gray-300">{client.projectName || 'N/A'}</span></p>

        <div className="flex items-center justify-between mb-3">
          <span className={`text-sm font-medium ${countdownColor}`}>{countdownText}</span>
        ...
        {client.monthlyRevenue && (
          <p className="text-[#13b973] text-sm font-medium mb-3">${Number(client.monthlyRevenue).toLocaleString()}/mo</p>
        )}

        <div className="flex items-center gap-2 flex-wrap">
          <button onClick={() => setNoteModal(true)} className="px-3 py-1.5 text-xs bg-dark-200/50 text-gray-400 hover:text-white hover:bg-dark-200 rounded-lg transition-all">+ Note</button>
          <button onClick={() => setShowNotes(!showNotes)} className="px-3 py-1.5 text-xs bg-dark-200/50 text-gray-400 hover:text-white hover:bg-dark-200 rounded-lg transition-all">
            Notes ({(client.notes || []).length})
          </button>
          <button onClick={markComplete} className="px-3 py-1.5 text-xs bg-[#13b973]/10 text-[#13b973] hover:bg-[#13b973]/20 rounded-lg transition-all ml-auto">Mark Complete</button>
        </div>

        {showNotes && (client.notes || []).length > 0 && (
          <div className="mt-3 pt-3 border-t border-white/5 space-y-2 max-h-40 overflow-y-auto">
            {(client.notes || []).map((n) => (
              <div key={n.id} className="text-xs text-gray-400">
                <span className="text-gray-600">{new Date(n.date).toLocaleDateString()}</span> &mdash; {n.text}
              </div>
            ))}
          </div>
        )}
  ...
  return (
    <ToolLayout title="Multi-Client Command Center" description="Dashboard to manage all your clients, projects, deadlines, and invoices in one place.">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        {[
          { label: 'Active Clients', value: stats.active, color: 'text-[#13b973]' },
          { label: 'Due This Week', value: stats.dueThisWeek, color: 'text-yellow-400' },
          { label: 'Overdue Items', value: stats.overdue, color: 'text-red-400' },
          { label: 'Monthly Revenue', value: `$${stats.totalRevenue.toLocaleString()}`, color: 'text-[#13b973]' },
        ].map((s) => (
          <div key={s.label} className="bg-dark-100/50 border border-white/5 rounded-xl p-4 text-center">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
        <div className="flex items-center gap-2 flex-wrap">
          {filters.map((f) => (
            <button
              ...
              className={`px-4 py-2 text-sm rounded-xl transition-all ${filter === f.key ? 'bg-[#13b973] text-white' : 'bg-dark-200/50 text-gray-400 hover:text-white hover:bg-dark-200'}`}
            >
              {f.label}
            </button>
          ))}
        ...
        <div className="flex items-center gap-3 w-full sm:w-auto">
          <input
            ...
            className="flex-1 sm:w-48 bg-dark-200/50 border border-white/10 rounded-xl px-4 py-2 text-white text-sm placeholder-gray-500 focus:outline-none focus:border-[#13b973]/50"
          />
          <select
            value={sort}
            onChange={(e) => setSort(e.target.value)}
            className="bg-dark-200/50 border border-white/10 rounded-xl px-3 py-2 text-gray-300 text-sm focus:outline-none focus:border-[#13b973]/50"
          >
            {sorts.map((s) => (
              <option key={s.key} value={s.key}>Sort: {s.label}</option>
            ))}
          </select>
          <button onClick={openAdd} className="px-5 py-2 bg-[#13b973] hover:bg-[#13b973]/80 text-white rounded-xl font-medium transition-colors text-sm whitespace-nowrap">
            + Add Client
          </button>
        </div>
      </div>
      ...
      <Modal open={showModal} onClose={() => setShowModal(false)} title={editClient ? 'Edit Client/Project' : 'Add Client/Project'}>
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <select
                ...
                className="w-full bg-dark-200/50 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-[#13b973]/50"
              >
            ...
            <div>
              <label className="block text-sm text-gray-400 mb-1.5">Invoice Status</label>
              <select
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-sm text-gray-400 mb-1.5">Monthly Revenue ($)</label>
              <input
                type="number"
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            <button onClick={handleSave} className="px-6 py-2.5 bg-[#13b973] hover:bg-[#13b973]/80 text-white rounded-xl font-medium transition-colors">
